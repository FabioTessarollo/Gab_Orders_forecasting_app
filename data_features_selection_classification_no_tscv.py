...
scores_df = pd.DataFrame(columns = ['brand', 'features', 'score']) 
...
parameters = {
    ...
    'scale_pos_weight': [2, 3, 4]  # Important for handling imbalance  1 ,  4, 5
}
...
def get_feature_comb_score(brand, features_comb):
    ...
    return features_by_importance, final_score


for brand in perimeter:
    print(f"\n###########################Brand {brand}:")
    initial_features, score = get_feature_comb_score(brand, initial_features_comb)
    best = {'brand': brand, 'features': [initial_features], 'score': score}
    max_score = score
    for n in range(2, 11):
        comb = initial_features[:n]
        features, score = get_feature_comb_score(brand, comb)
        if score > max_score:
            best = {'brand': brand, 'features': [features], 'score': score}
            max_score = score
